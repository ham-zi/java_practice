...
public class LunchMenu {
	...
	public void findLunchMenu() {
		...
		System.out.println("2. 빅맥 세트(8000)");
...
//data type(자료형)| Variable(변수식별자, 변수명)	
		
		// 초기화 (Initialization)
		// 변수는 자신이 선언된 {Scope}에서만 사용할 수 있음. 지역변수
		...
		1. kweyWord(예약어)는 식별자로 사용할 수 없음
		2. 특수문자 '$','_' 만 사용 가능
		3. 공백 포함할 수 없음
		4. 대, 소문자를 구분하고 길이 제한 없음
		5. 숫자가 앞에 오는건 불가능 
		...
		- 첫 글짜는 대문자로 표기
		- 연결된 단어가 바뀔 때도 첫글짜 대문자로 표기
		- 명사, 형용사를 서술적으로 연결해서 사용
		
		
		
		
		2.변수 명명 규칙 (camelCase)
		- 명사적 의미를 갖게 만들어 줌
		- 첫글자를 소문자로 표기, 연결된 단어들의 첫글자를 대문자로 표기
		 ...
		 - 동사적 의미를 갖게 만들어줌
		 - 첫 첫글자를 소문자로 표기, 연결된 단어들의 첫글자를 대문자로 표기
		 - 메소드에는 _를 사용하지 않음
		...
		*/
		...
		기본자료형 byte short char int long boolean float double
		정수형 byte short int long 
		- long 사용할 때는 대입할 숫자뒤에 L
		실수형 float double
		문자형 char
		불린형 boolean                   
		...
		*/
		...
		// 지역변수(local variable)은 초기화하지 않으면 사용할 수 없음.
		
		String str = "문자열";
		byte byteNumber = 'A';
		short shortNumber = byteNumber;
		int intNumber = 1123123;
		long longNumber = 1234123L;
		char character = 'A';
		boolean isFalse = false;
		float floatNumber = 3.14F;
		double doubleNumber = 1.2341;
		...
		// 단일문자에는 반드시 단일인용문구를 적는다. 'A'
		// System.out.println(); => 개행한다
		
		System.out.printf("%b %d %s\n",isTrue,byteNumber,str);
		
		System.out.printf("%2$.3f 와 %1$.1f\n", floatNumber, doubleNumber);
		// 논리자료형 %b
		// 정수자료형 %d decimal(10진법의)
		// 실수자료형 %f
		// 단일문자자료형 %c
		// 참조자료형String %s
		
		//상수 => 프로그래밍 언어
		//변수인데, 값을 변경하지 않을 변수 공간
